...
fn mainInternal() !void {
    ...
    try benchCpu(w);
    if (@import("build_consts").gpu_support) try benchGpu(w);
}
...
fn printBenchmarkHeader(out: *std.Io.Writer) !void {
    try out.print("slimy benchmark v{f}\n\n", .{version.version});
}
...
fn benchGpu(w: *std.Io.Writer) !void {
    ...
    // 3x ~5s GPU benchmark
    try w.writeAll("Testing GPU");
    var gpu_rate: u128 = approx_gpu_rate;
    for (0..3) |i| {
        try w.writeByte('.');
        gpu_rate += try benchGpuIter(&gpu, gpu_rate / (i + 1), 5);
    }
...
fn benchGpuIter(gpu: *slimy.gpu.Context, rate: u128, target_secs: u8) !u128 {
